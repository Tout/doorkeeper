...
module Doorkeeper::OAuth
  ...
  describe AccessTokenRequest, "grant for a confidential client with no redirect_uri in the grant" do
    let(:client) { Factory(:application, :public => false) }
    let(:grant)  { Factory(:access_grant, :application => client, :redirect_uri => nil) }
    let(:params) {
      {
        :client_id     => client.uid,
        :client_secret => client.secret,
